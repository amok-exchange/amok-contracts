...
describe("AMT", function () {
  ...
  it("inits", async () => {
    ...
    expect(await gmt.balanceOf(wallet.address)).eq(
  ...
  it("setGov", async () => {
    await expect(gmt.connect(user0).setGov(user1.address)).to.be.revertedWith(
      "AMT: forbidden"
    );
  ...
  it("addAdmin", async () => {
    await expect(gmt.connect(user0).addAdmin(user1.address)).to.be.revertedWith(
      "AMT: forbidden"
    );
  ...
  it("removeAdmin", async () => {
    await expect(
      gmt.connect(user0).removeAdmin(user1.address)
    ).to.be.revertedWith("AMT: forbidden");
  ...
  it("setNextMigrationTime", async () => {
    await expect(
      gmt.connect(user0).setNextMigrationTime(1000)
    ).to.be.revertedWith("AMT: forbidden");
    ...
    await expect(gmt.setNextMigrationTime(999)).to.be.revertedWith(
      "AMT: invalid _migrationTime"
    );
  ...
  it("beginMigration", async () => {
    await expect(gmt.connect(user0).beginMigration()).to.be.revertedWith(
      "AMT: forbidden"
    );
    ...
    await expect(gmt.connect(user0).beginMigration()).to.be.revertedWith(
      "AMT: migrationTime not yet passed"
    );
  ...
  it("addBlockedRecipient", async () => {
    await expect(
      gmt.connect(user0).addBlockedRecipient(user1.address)
    ).to.be.revertedWith("AMT: forbidden");
  ...
  it("removeBlockedRecipient", async () => {
    await expect(
      gmt.connect(user0).removeBlockedRecipient(user1.address)
    ).to.be.revertedWith("AMT: forbidden");
  ...
  it("addMsgSender", async () => {
    await expect(
      gmt.connect(user0).addMsgSender(user1.address)
    ).to.be.revertedWith("AMT: forbidden");
  ...
  it("removeMsgSender", async () => {
    await expect(
      gmt.connect(user0).removeMsgSender(user1.address)
    ).to.be.revertedWith("AMT: forbidden");
  ...
  it("withdrawToken", async () => {
    ...
    await expect(
      gmt.connect(user0).withdrawToken(token.address, user1.address, 1000)
    ).to.be.revertedWith("AMT: forbidden");
  ...
  it("transfer", async () => {
    ...
    expect(await gmt.balanceOf(wallet.address)).eq(
      expandDecimals(999 * 1000, 18)
    );
    expect(await gmt.balanceOf(user0.address)).eq(expandDecimals(1000, 18));

    await expect(
      gmt.connect(user0).transfer(user1.address, expandDecimals(1001, 18))
    ).to.be.revertedWith("AMT: transfer amount exceeds balance");
  });
  ...
  it("transferFrom", async () => {
    ...
    expect(await gmt.allowance(wallet.address, user0.address)).eq(
    ...
    await gmt
      .connect(user0)
      .transferFrom(wallet.address, user1.address, expandDecimals(1000, 18));
    expect(await gmt.allowance(wallet.address, user0.address)).eq(
    ...
    await expect(
      gmt
        .connect(user0)
        .transferFrom(wallet.address, user1.address, expandDecimals(1001, 18))
    ).to.be.revertedWith("AMT: transfer amount exceeds allowance");
  });
  ...
  it("allows migrations", async () => {
    ...
    await expect(
      gmt
        .connect(user0)
        .transferFrom(wallet.address, user1.address, expandDecimals(500, 18))
    ...
    await expect(
      gmt
        .connect(user0)
        .transferFrom(wallet.address, user1.address, expandDecimals(500, 18))
    ).to.be.revertedWith("AMT: forbidden recipient");

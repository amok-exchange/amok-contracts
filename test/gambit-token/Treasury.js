...
describe("Treasury", function () {
  ...
  beforeEach(async () => {
    ...
    gmt = await deployContract("AMT", [expandDecimals(1000 * 1000, 18)]);
    ...
    await treasury.initialize(
      [gmt.address, busd.address, router.address, fund.address],
      [
        gmtPresalePrice,
        gmtListingPrice,
        busdSlotCap,
  ...
  it("initialize", async () => {
    ...
    expect(await treasury.amt()).eq(gmt.address);
  ...
  it("setGov", async () => {
    await expect(
      treasury.connect(user0).setGov(user1.address)
    ).to.be.revertedWith("Treasury: forbidden");
  ...
  it("setFund", async () => {
    await expect(
      treasury.connect(user0).setFund(user2.address)
    ).to.be.revertedWith("Treasury: forbidden");
  ...
  it("extendUnlockTime", async () => {
    await expect(
      treasury.connect(user0).extendUnlockTime(unlockTime + 100)
    ).to.be.revertedWith("Treasury: forbidden");

    await expect(
      treasury.extendUnlockTime(unlockTime - 100)
  ...
  it("addWhitelists", async () => {
    ...
    await expect(
      treasury.connect(user0).addWhitelists(whitelist)
    ).to.be.revertedWith("Treasury: forbidden");
  ...
  it("removeWhitelists", async () => {
    ...
    await expect(
      treasury.connect(user0).removeWhitelists(whitelist)
    ).to.be.revertedWith("Treasury: forbidden");
  ...
  it("updateWhitelist", async () => {
    ...
    await expect(
      treasury.connect(user0).updateWhitelist(user0.address, user3.address)
    ).to.be.revertedWith("Treasury: forbidden");

    await expect(
      treasury.updateWhitelist(user3.address, user0.address)
  ...
  it("swap", async () => {
    ...
    await expect(treasury.connect(user3).swap(2000)).to.be.revertedWith(
      "Treasury: forbidden"
    );

    await expect(treasury.connect(user0).swap(0)).to.be.revertedWith(
      "Treasury: invalid _busdAmount"
    );

    await expect(treasury.connect(user0).swap(2000)).to.be.revertedWith(
      "ERC20: transfer amount exceeds allowance"
    );

    await busd
      .connect(user0)
      .approve(treasury.address, expandDecimals(1000 * 1000, 18));
    await expect(treasury.connect(user0).swap(2000)).to.be.revertedWith(
      "AMT: transfer amount exceeds balance"
    );
    ...
    expect(await busd.balanceOf(user0.address)).eq(
      expandDecimals(1000 * 1000, 18)
    );
    expect(await gmt.balanceOf(treasury.address)).eq(
      expandDecimals(1000 * 1000, 18)
    );
    ...
    expect(await gmt.balanceOf(treasury.address)).eq(
      "999999999999999999999556"
    );
    expect(await gmt.balanceOf(user0.address)).eq(444);

    await expect(
      gmt.connect(user0).transfer(user1.address, 100)
    ).to.be.revertedWith("AMT: forbidden msg.sender");
  ...
  it("validates swap.busdSlotCap", async () => {
    ...
    expect(await gmt.balanceOf(treasury.address)).eq(
      "999777777777777777777778"
    );
    expect(await gmt.balanceOf(user0.address)).eq("222222222222222222222");

    await expect(
      treasury.connect(user0).swap(expandDecimals(1001, 18))
    ).to.be.revertedWith("Treasury: busdSlotCap exceeded");

    await treasury.connect(user0).swap(expandDecimals(1000, 18));

    expect(await treasury.swapAmounts(user0.address)).eq(
      expandDecimals(2000, 18)
    );
    ...
    await expect(treasury.connect(user0).swap("1")).to.be.revertedWith(
  ...
  it("validates swap.busdHardCap", async () => {
    ...
    expect(await treasury.busdReceived()).eq(0);
    await expect(
      treasury.connect(user0).swap(expandDecimals(901 * 1000, 18))
    ).to.be.revertedWith("Treasury: busdHardCap exceeded");

    await expect(
      treasury.connect(user0).swap(expandDecimals(900 * 1000, 18))
    ).to.be.revertedWith("Treasury: busdSlotCap exceeded");
    ...
    await expect(
      treasury.connect(user0).swap(expandDecimals(899 * 1000, 18))
    ).to.be.revertedWith("Treasury: busdHardCap exceeded");

    await expect(
      treasury.connect(user0).swap(expandDecimals(898 * 1000, 18))
    ).to.be.revertedWith("Treasury: busdSlotCap exceeded");
  });
  ...
  it("validates swap.isSwapActive", async () => {
    ...
    await expect(
      treasury.connect(user0).swap(expandDecimals(1000, 18))
  ...
  it("addLiquidity", async () => {
    ...
    await expect(treasury.connect(user0).addLiquidity()).to.be.revertedWith(
      "Treasury: forbidden"
    );
    ...
    await expect(treasury.addLiquidity()).to.be.revertedWith(
      "Treasury: liquidity already added"
    );
  });

  it("withdrawToken", async () => {
    await expect(
      treasury.connect(user0).withdrawToken(busd.address, user1.address, 1000)
    ).to.be.revertedWith("Treasury: forbidden");

    await expect(
      treasury.withdrawToken(busd.address, user1.address, 1000)
  ...
  it("increaseBusdBasisPoints", async () => {
    await expect(
      treasury.connect(user0).increaseBusdBasisPoints(4000)
    ).to.be.revertedWith("Treasury: forbidden");

    await expect(treasury.increaseBusdBasisPoints(4000)).to.be.revertedWith(
  ...
  it("endSwap", async () => {
    await expect(treasury.connect(user0).endSwap()).to.be.revertedWith(
      "Treasury: forbidden"
    );

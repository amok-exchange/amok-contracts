...
describe("Bridge", function () {
  ...
  beforeEach(async () => {
    gmx = await deployContract("AMOK", []);
    wgmx = await deployContract("AMOK", []);
  ...
  it("wrap, unwrap", async () => {
    ...
    await expect(
      bridge.connect(user0).wrap(200, user1.address)
    ).to.be.revertedWith("BaseToken: transfer amount exceeds allowance");

    await expect(
      bridge.connect(user0).wrap(100, user1.address)
  ...
  it("withdrawToken", async () => {
    ...
    await expect(
      bridge.connect(user0).withdrawToken(gmx.address, user1.address, 100)
    ).to.be.revertedWith("Governable: forbidden");

    await expect(
      bridge.connect(user0).setGov(user0.address)
    ).to.be.revertedWith("Governable: forbidden");

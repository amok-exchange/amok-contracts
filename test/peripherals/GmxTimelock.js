...
describe("GmxTimelock", function () {
  ...
  it("mint", async () => {
    const gmx = await deployContract("AMOK", []);
    await expect(
      timelock.connect(user0).mint(gmx.address, 900)
    ).to.be.revertedWith("GmxTimelock: forbidden");

    await expect(
      timelock.connect(wallet).mint(gmx.address, 900)
    ...
    await expect(
      timelock.connect(wallet).mint(gmx.address, expandDecimals(1001, 18))
  ...
  it("setFundingRate", async () => {
    ...
    await expect(
      timelock.connect(wallet).setFundingRate(vault.address, 59 * 60, 100, 100)

...
describe("BatchSender", function () {
  ...
  beforeEach(async () => {
    ...
    gmt = await deployContract("AMT", [1500]);
  ...
  it("send", async () => {
    ...
    await expect(
      batchSender.connect(user0).send(gmt.address, accounts, amounts)
    ).to.be.revertedWith("BatchSender: forbidden");

    await gmt.approve(batchSender.address, 1000);

    await expect(
      batchSender.connect(wallet).send(gmt.address, accounts, amounts)
